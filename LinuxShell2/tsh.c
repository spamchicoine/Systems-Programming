	...
	if(builtin_cmd(argv)){		//Call builtin_cmd to return true and run command if its built in
		...
		if((pid = fork()) == 0){	//Create child to run process
			...
			while (argv[i] != NULL){
				...
				// Redirect output
				if (strcmp(argvcopy[i],">")==0){
					
					int newstdout = open(argvcopy[i+1], O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRWXG|S_IRWXO);
					if (newstdout == -1){
				...
				// redirect input
				else if (strcmp(argvcopy[i],"<")==0){
					int newstdin = open(argvcopy[i+1], O_RDONLY, S_IRWXU|S_IRWXG|S_IRWXO);
					if (newstdin == -1){
						app_error("open error");
					}
				...
				// Append output
					...
					int newstdout = open(argvcopy[i+1], O_WRONLY|O_CREAT|O_APPEND, S_IRWXU|S_IRWXG|S_IRWXO);
					if (newstdout == -1){
						app_error("open error");
					}
				...
				// Error output
				else if (strcmp(argvcopy[i],"2>")==0){
				
					int newstderr = open(argvcopy[i+1], O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRWXG|S_IRWXO);
					if (newstderr == -1){
						app_error("open error");
					}
				...
				// Pipe time
					...
					if (pid > 0){
						...
						// Run command to left of pipe (i-2), without rest of cmdline
							// Need to set everything from i onwards to NULL
						char *argvLeft[3];
						argvLeft[0] = argvcopy[i-2]; argvLeft[1] = argvcopy[i-1]; argvLeft[2] = NULL;

						if (execve(argvLeft[0], argvLeft, environ ) < 0) {		//Execute process
					...
					// Child
						...
						int j = i + 1;
						int anotherPipe = 0;
						// Loop through rest of argv, see if there is a second pipe
						while (argv[j] != NULL){
							if (strcmp(argv[j], "|") == 0){
								anotherPipe = 1;
								break;
						...
						if (anotherPipe){
							pid_t pid2; int secondPipe[2];
							if (pipe(secondPipe) < 0){
								app_error("Pipe error");
							}
							if ((pid2 = fork()) == 0){ // PARENT
								// Redirect stdout from prev command to new pipe stdin
								if (dup2(secondPipe[1],1) == -1){
									app_error("dup2 error");
								}
								char *argvLeft[3];
								argvLeft[0] = argv[j-2]; argvLeft[1] = argv[j-1]; argvLeft[2] = NULL;
								if (execve(argvLeft[0], argvLeft, environ ) < 0) {		//Execute process
								...
								close(secondPipe[1]);
								// Redirect stdout from pipe to stdin
								if (dup2(secondPipe[0],0) == -1){
									app_error("dup2 error");
								}
								char *argvRight[3];
								argvRight[0] = argv[j+1]; argvRight[1] = argv[j+2]; argvRight[2] = NULL;
								if (execve(argvRight[0], argvRight, environ ) < 0) {		//Execute process
									printf("%s: Command not found 2.\n", argvRight[0]);
									exit(0);
								}
								close(secondPipe[0]);
							}
							// Redirect stdout to new pipe
							// fork, middle process is parent
						}else{
							char *argvRight[3];
							argvRight[0] = argvcopy[i+1]; argvRight[1] = argvcopy[i+2]; argvRight[2] = NULL;
							if (execve(argvRight[0], argvRight, environ ) < 0) {		//Execute process
								printf("%s: Command not found 2.\n", argvRight[0]);
								exit(0);
							}
			...
			if (execve(argvcopy[0], argv, environ ) < 0) {		//Execute process
