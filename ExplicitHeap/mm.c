...
static void insertFreeBlock(BlockInfo* freeBlock) {
  ...
  freeBlock->prev = NULL;	//This was commented out but seems neccesary otherwise really weird stuff happens in the short traces
...
void* mm_malloc (size_t size) {
	...
	// Search for free block, request space if not found
	BlockInfo* mallocBlock = searchFreeList(reqSize);
	if(mallocBlock == NULL){
		requestMoreSpace(reqSize);
		mallocBlock = searchFreeList(reqSize);
	}
	// SPLIT
	if (SIZE(mallocBlock->sizeAndTags) - reqSize >= MIN_BLOCK_SIZE){
		printf("splitting\n");
		// block size - req size > minblock size
		BlockInfo* splitBlock = UNSCALED_POINTER_ADD(mallocBlock, reqSize); //needs to be initialized to block location + minblock size
		size_t newSize = ((SIZE(mallocBlock->sizeAndTags) - reqSize + 1) >> 1) << 1;
		splitBlock->sizeAndTags = newSize;
		// Split block will always have its preceding used, preceding is mallocBlock being returned
		splitBlock->sizeAndTags |= TAG_PRECEDING_USED;
		splitBlock->sizeAndTags &= ~TAG_USED;

		//Set footer
		size_t* footer = UNSCALED_POINTER_SUB(UNSCALED_POINTER_ADD(splitBlock,newSize),WORD_SIZE);
		*footer = splitBlock->sizeAndTags;

		insertFreeBlock(splitBlock);
	}
	
	mallocBlock->sizeAndTags = reqSize;
	mallocBlock->sizeAndTags |= TAG_USED;
	mallocBlock->sizeAndTags |= TAG_PRECEDING_USED;
	removeFreeBlock(mallocBlock);
	
	printf("\nafter malloc\n");
	fflush(stdout);
	examine_heap();
	
	return UNSCALED_POINTER_ADD(mallocBlock,WORD_SIZE); 
}

void mm_free (void *ptr) {
	BlockInfo* blockInfo = UNSCALED_POINTER_SUB(ptr, WORD_SIZE);	//Get address of struct
	int blocksize = SIZE(blockInfo->sizeAndTags);	//Get size of block
	
	size_t payloadSize = blocksize - MIN_BLOCK_SIZE;	//Get size of payload in bytes
	
	blockInfo->sizeAndTags &= ~TAG_USED;	//Set block's used tag to 0
	
	blockInfo->sizeAndTags |= TAG_PRECEDING_USED;	/*THIS IS WHAT NEEDS CHANGE THE PRECEDING IS NOT ALWAYS USED*/
	
	size_t* footerlocation = UNSCALED_POINTER_SUB(UNSCALED_POINTER_ADD(blockInfo, blocksize), WORD_SIZE);	//Get footer location
	*footerlocation = blockInfo->sizeAndTags;	//Set footer equal to header
	
	BlockInfo* followingBlock = UNSCALED_POINTER_ADD(blockInfo, blocksize);	//Address of next block
	followingBlock->sizeAndTags &= ~TAG_PRECEDING_USED;	//Set following block's header preceeding tag to 0
	
	insertFreeBlock(blockInfo);	//Insert block into free list
	
	printf("\nafter insert free block\n");
	fflush(stdout);
	examine_heap();
	
	coalesceFreeBlock(blockInfo);	//Coalesce with adjacent free blocks
	// Implement mm_free.  You can change or remove the declaraions
	// above.  They are included as minor hints.
	
	printf("\nafter coalesce free block\n");
	fflush(stdout);
	examine_heap();
